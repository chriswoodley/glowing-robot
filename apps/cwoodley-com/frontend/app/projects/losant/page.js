...
function LosantPage() {
  return (
    <div>
      <Link href="https://app.losant.com" target="_blank">
        <p className="my-0 text-right" >
          Visit{' '}
          <Icon
            glyph="external-link"
            title="Visit app.losant.com"
            size="24"
      ...
      <p>Losant&apos;s enterprise IoT Platform enables customers to create experiences with{' '}
      connected IoT devices using the platform&apos;s low code workflow engine. Examples of{' '}
      the types of solutions customers can build are (but not limited to):</p>
      ...
      <ul>
        ...
        <li>
          Develop new features and continually maintain and enhance existing functionality of{' '}
          the IoT platform.
        </li>
        <li>
      ...
      <p>
        ...
        and many unit tests were written with Enzyme. I spearheaded and delegated the effort{' '}
        to handle both code deprecation’s and to migrate Enzyme tests over to the React{' '}
        Testing Library.
      </p>

      <p>
        I was also responsible for maintaining our Front-end build tools. The Losant Platform{' '}
        used Webpack 4 for local and production environment builds for many years. The Webpack{' '}
        plugins became obsolete. To ensure the continuity of Losant’s platform, I had the{' '}
        challenge of upgrading to Webpack 5. Through careful understanding of Webpack’s{' '}
        documentation and information that was available on the web, I was able to migrate the{' '}
        configurations for each environment. In that process, I was able to upgrade all of the{' '}
        Webpack plugins that were being used.
      </p>

      <p>
        Other than library upgrades and maintaining build tools, I was responsible for ensuring{' '}
        that our Unit Tests were healthy. The platform’s front-end component Unit Tests did not{' '}
        mock API requests. The Unit Tests also did not tear down Fake Timers or handle{' '}
        Promises after each test has completed. As a result, Jest errors, timeouts, and random{' '}
        test failures occurred during test runs.
      </p>
      ...
      <p>
        ...
        multiple organizations used within a single enterprise company. It is intended for use{' '}
        by Administrators at a large scale, Resellers, Losant partners, and Enterprise{' '}
        customers that have a private installation of the Losant Platform. Apart from adding{' '}
        new features to the platform, I am responsible for the overall maintenance of the User{ ''}
        Interface. My role as Senior Front-end Software Engineer:
      </p>
